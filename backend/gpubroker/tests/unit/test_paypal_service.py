...
class TestPayPalConfigurationStatus:
    ...
    def test_get_config_status_returns_correct_structure(self):
        ...
        with patch.object(PayPalService, '__init__', lambda self: None):
            ...
            type(service).is_configured = property(lambda self: bool(self.client_id and self.client_secret))
...
class TestPayPalOrderCreation:
    ...
    def test_create_payment_for_subscription_builds_correct_urls(self):
        ...
        with patch.object(PayPalService, '__init__', lambda self: None):
            ...
            captured_urls = {}
            def mock_create_order(email, plan, amount_usd, return_url, cancel_url, ruc='', name=''):
            ...
            assert captured_urls['return_url'] == 'https://gpubroker.example.com/payment/paypal/callback'
...
class TestAPIResponseStructure:
    ...
    **Validates: Requirements 2.1, 3.1, 4.1, 5.1**
    ...
    def test_property_api_response_has_success_field(self, endpoint: str):
        ...
        expected_structures = {
            'dashboard': {
                'required_fields': ['pods', 'revenue', 'plans', 'customers', 'alerts', 'activity'],
            ...
            'costs': {
                'required_fields': ['success', 'aws_costs', 'profitability', 'per_pod_estimate'],
                'profitability_fields': ['revenue', 'profit', 'margin_percent', 'is_profitable'],
            },
        }
        ...
        if endpoint in ['pods', 'customers', 'billing']:
            list_field = f"{endpoint.rstrip('s')}_is_list" if endpoint != 'billing' else 'transactions_is_list'
            if endpoint == 'pods':
...
class TestPayPalOrderCreationProperty:
    ...
    **Validates: Requirements 1.1, 1.2**
    ...
    @settings(max_examples=100)
    def test_property_order_creation_response_structure(self, email: str, plan: str, amount: float):
        """
        ...
        with patch.object(PayPalService, '__init__', lambda self: None):
            ...
            if result['success']:
                ...
                assert result['approval_url'].startswith('https://www.paypal.com') or \
                       result['approval_url'].startswith('https://www.sandbox.paypal.com')
            else:
    ...
    @given(st.floats(min_value=-1000.0, max_value=0.0, allow_nan=False, allow_infinity=False))
    ...
    def test_property_invalid_amount_rejected(self, amount: float):
        ...
        with patch.object(PayPalService, '__init__', lambda self: None):
            ...
            def mock_create_order(email, plan, amount_usd, return_url, cancel_url, ruc='', name=''):
                ...
                return {'success': True, 'approval_url': 'https://www.sandbox.paypal.com/test'}
