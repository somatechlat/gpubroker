...
def run_tests():
    ...
    try:
        user_model_path = PROJECT_ROOT / "backend" / "gpubroker" / "gpubrokerpod" / "gpubrokerapp" / "apps" / "auth_app" / "models.py"
        assert user_model_path.exists(), f"User model file should exist at {user_model_path}"
        
        model_content = user_model_path.read_text()
        assert "class User(" in model_content, "User model class should be defined"
        
        expected_fields = ['email', 'password_hash', 'full_name', 'organization', 'is_active', 'is_verified']
        for field in expected_fields:
            assert field in model_content, f"User model should have '{field}' field"
        
        assert "def set_password(" in model_content, "User model should have set_password method"
        assert "def check_password(" in model_content, "User model should have check_password method"
    ...
    try:
        ...
        assert "passlib" in model_content or "argon2" in model_content, "User model should use passlib or argon2"
    ...
    try:
        provider_model_path = PROJECT_ROOT / "backend" / "gpubroker" / "gpubrokerpod" / "gpubrokerapp" / "apps" / "providers" / "models.py"
        assert provider_model_path.exists(), f"Provider model file should exist"
        
        model_content = provider_model_path.read_text()
        assert "class Provider(" in model_content, "Provider model class should be defined"
        
        expected_fields = ['name', 'display_name', 'api_base_url', 'status', 'reliability_score']
        for field in expected_fields:
    ...
    try:
        ...
        assert "class GPUOffer(" in model_content, "GPUOffer model class should be defined"
        
        expected_fields = ['gpu_type', 'gpu_memory_gb', 'cpu_cores', 'ram_gb', 'price_per_hour', 'region']
        for field in expected_fields:
    ...
    try:
        billing_model_path = PROJECT_ROOT / "backend" / "gpubroker" / "gpubrokerpod" / "gpubrokerapp" / "apps" / "billing" / "models.py"
        assert billing_model_path.exists(), f"Billing model file should exist"
        
        model_content = billing_model_path.read_text()
        assert "class Subscription(" in model_content, "Subscription model should be defined"
        assert "class PaymentMethod(" in model_content or "class Payment(" in model_content, "Payment/PaymentMethod model should be defined"
    ...
    try:
        stripe_service_path = PROJECT_ROOT / "backend" / "gpubroker" / "gpubrokerpod" / "gpubrokerapp" / "apps" / "billing" / "stripe_service.py"
        email_service_path = PROJECT_ROOT / "backend" / "gpubroker" / "gpubrokerpod" / "gpubrokerapp" / "apps" / "billing" / "email_service.py"
        ...
        assert "def create_customer(" in stripe_content, "stripe_service should have create_customer function"
    ...
    try:
        deployment_model_path = PROJECT_ROOT / "backend" / "gpubroker" / "gpubrokerpod" / "gpubrokerapp" / "apps" / "deployment" / "models.py"
        assert deployment_model_path.exists(), "Deployment model file should exist"
        
        model_content = deployment_model_path.read_text()
        assert "class PodDeploymentConfig(" in model_content or "class Deployment(" in model_content, "Deployment/PodDeploymentConfig model should be defined"
        
        expected_fields = ['user_id', 'status', 'gpu_type']
        for field in expected_fields:
            assert field in model_content, f"Deployment model should have '{field}' field"
    ...
    try:
        deployment_services_path = PROJECT_ROOT / "backend" / "gpubroker" / "gpubrokerpod" / "gpubrokerapp" / "apps" / "deployment" / "services.py"
        assert deployment_services_path.exists(), "Deployment services file should exist"
        
        services_content = deployment_services_path.read_text()
        # Check for any deployment-related functions
        assert "def " in services_content, "deployment services should have functions defined"
    ...
    try:
        pod_config_model_path = PROJECT_ROOT / "backend" / "gpubroker" / "gpubrokerpod" / "gpubrokerapp" / "apps" / "pod_config" / "models.py"
        ...
        assert "class " in model_content and "models.Model" in model_content, "Pod config should have model classes defined"
    ...
    try:
        dashboard_services_path = PROJECT_ROOT / "backend" / "gpubroker" / "gpubrokerpod" / "gpubrokerapp" / "apps" / "dashboard" / "services.py"
        ...
        assert "def get_" in services_content, "dashboard services should have getter functions"
    ...
    try:
        dashboard_schemas_path = PROJECT_ROOT / "backend" / "gpubroker" / "gpubrokerpod" / "gpubrokerapp" / "apps" / "dashboard" / "schemas.py"
    ...
    try:
        websocket_routing_path = PROJECT_ROOT / "backend" / "gpubroker" / "gpubrokerpod" / "gpubrokerapp" / "apps" / "websocket_gateway" / "routing.py"
        assert websocket_routing_path.exists(), "Websocket routing file should exist"
        
        routing_content = websocket_routing_path.read_text()
        assert "websocket_urlpatterns" in routing_content, "websocket_gateway routing should have websocket_urlpatterns"
    ...
    try:
        websocket_consumers_path = PROJECT_ROOT / "backend" / "gpubroker" / "gpubrokerpod" / "gpubrokerapp" / "apps" / "websocket_gateway" / "consumers.py"
        assert websocket_consumers_path.exists(), "Websocket consumers file should exist"
        
        consumers_content = websocket_consumers_path.read_text()
        assert "Consumer" in consumers_content, "websocket_gateway consumers should have Consumer classes"
        assert "def connect(" in consumers_content, "Consumer should have connect method"
        assert "def disconnect(" in consumers_content, "Consumer should have disconnect method"
    ...
    try:
        provider_api_path = PROJECT_ROOT / "backend" / "gpubroker" / "gpubrokerpod" / "gpubrokerapp" / "apps" / "providers" / "api.py"
    ...
    try:
        billing_api_path = PROJECT_ROOT / "backend" / "gpubroker" / "gpubrokerpod" / "gpubrokerapp" / "apps" / "billing" / "api.py"
    ...
    try:
        deployment_api_path = PROJECT_ROOT / "backend" / "gpubroker" / "gpubrokerpod" / "gpubrokerapp" / "apps" / "deployment" / "api.py"
    ...
    try:
        dashboard_api_path = PROJECT_ROOT / "backend" / "gpubroker" / "gpubrokerpod" / "gpubrokerapp" / "apps" / "dashboard" / "api.py"
    ...
    try:
        ...
        for app_module in pod_saas_apps:
            try:
                ...
                raise AssertionError(f"Failed to import POD SaaS app '{app_module}': {e}")
