    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if data.get("status") == "healthy":
                results.success(
                    "Health Check",
                    f'DB: {data.get("database")}, Cache: {data.get("cache")}',
                )
            else:
...
async def test_register(
    client: httpx.AsyncClient, results: IntegrationTestResult
) -> dict | None:
    ...
    try:
        ...
        if response.status_code in (200, 201):
            ...
            results.success(
                "User Registration", "User already exists (expected in re-runs)"
            )
            return {"email": TEST_USER["email"]}
        else:
            results.failure(
                "User Registration", f"HTTP {response.status_code}: {response.text}"
            )
    except Exception as e:
...
async def test_login(
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            results.failure(
                "User Login", f"HTTP {response.status_code}: {response.text}"
            )
    except Exception as e:
...
async def test_token_refresh(
    client: httpx.AsyncClient, results: IntegrationTestResult, tokens: dict
) -> dict | None:
    """Test token refresh."""
    try:
        response = await client.post(
            f"{API_V2}/auth/refresh",
            json={
        ...
        if response.status_code == 200:
            ...
            results.failure(
                "Token Refresh", f"HTTP {response.status_code}: {response.text}"
            )
    except Exception as e:
...
async def test_me_endpoint(
    client: httpx.AsyncClient, results: IntegrationTestResult, access_token: str
):
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if data.get("email") == TEST_USER["email"]:
                results.success(
                    "Protected Endpoint (/auth/me)", f'User: {data.get("full_name")}'
                )
            else:
                results.failure(
                    "Protected Endpoint (/auth/me)", f"Email mismatch: {data}"
                )
        else:
            results.failure(
...
async def test_me_unauthorized(
    ...
    try:
        ...
        if response.status_code in (401, 403):
            results.success(
                "Unauthorized Access Rejected", f"HTTP {response.status_code}"
            )
        else:
            results.failure(
...
async def test_providers_list(
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            results.failure(
                "Provider Listing", f"HTTP {response.status_code}: {response.text}"
            )
    except Exception as e:
        results.failure("Provider Listing", str(e))


async def test_providers_filter(
    client: httpx.AsyncClient, results: IntegrationTestResult
):
    ...
    try:
        ...
        if response.status_code == 200:
            data = response.json()
            results.success(
                "Provider Filtering", f'Filtered results: {data.get("total", 0)}'
            )
        else:
            results.failure(
                "Provider Filtering", f"HTTP {response.status_code}: {response.text}"
            )
    except Exception as e:
...
async def test_kpi_overview(client: httpx.AsyncClient, results: IntegrationTestResult):
    ...
    try:
        ...
        if response.status_code == 200:
            data = response.json()
            results.success(
                "KPI Overview", f'Active providers: {data.get("active_providers", 0)}'
            )
        else:
            results.failure(
                "KPI Overview", f"HTTP {response.status_code}: {response.text}"
            )
    except Exception as e:
        results.failure("KPI Overview", str(e))


async def test_kpi_market_insights(
    client: httpx.AsyncClient, results: IntegrationTestResult
):
    ...
    try:
        ...
        if response.status_code == 200:
            data = response.json()
            results.success(
                "Market Insights", f'Total offers: {data.get("total_offers", 0)}'
            )
        else:
            results.failure(
                "Market Insights", f"HTTP {response.status_code}: {response.text}"
            )
    except Exception as e:
        results.failure("Market Insights", str(e))


async def test_math_benchmarks(
    client: httpx.AsyncClient, results: IntegrationTestResult
):
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            results.success(
                "GPU Benchmarks",
                f'Benchmarks: {len(data) if isinstance(data, list) else "N/A"}',
            )
        else:
            results.failure(
                "GPU Benchmarks", f"HTTP {response.status_code}: {response.text}"
            )
    except Exception as e:
        results.failure("GPU Benchmarks", str(e))


async def test_math_cost_per_token(
    client: httpx.AsyncClient, results: IntegrationTestResult
):
    """Test cost-per-token calculation."""
    try:
        response = await client.post(
            f"{API_V2}/math/cost-per-token",
            json={
                "price_per_hour": 1.50,
                "gpu_type": "RTX 4090",
                "model_size": "7B",
        ...
        if response.status_code == 200:
            ...
            results.failure(
                "Cost Per Token", f"HTTP {response.status_code}: {response.text}"
            )
    except Exception as e:
...
async def test_math_topsis(client: httpx.AsyncClient, results: IntegrationTestResult):
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            results.failure(
                "TOPSIS Ranking", f"HTTP {response.status_code}: {response.text}"
            )
    except Exception as e:
...
async def test_ai_health(client: httpx.AsyncClient, results: IntegrationTestResult):
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            results.failure(
                "AI Health", f"HTTP {response.status_code}: {response.text}"
            )
    except Exception as e:
...
async def test_provider_health(
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            results.success(
                "Provider Health",
                f'Adapters: {len(providers)} ({", ".join(providers[:5])}...)',
            )
        else:
            results.failure(
                "Provider Health", f"HTTP {response.status_code}: {response.text}"
            )
    except Exception as e:
        ...
        async with httpx.AsyncClient(timeout=30.0) as client:
            ...
            if response.status_code == 200:
                ...
                if offers:
                    ...
                    return (
                        True,
                        f"{len(offers)} offers (e.g., {gpu_name} @ ${price:.2f}/hr)",
                    )
...
async def run_integration_tests():
    ...
    print(f'\n{"="*60}')
    print("GPUBROKER INTEGRATION TEST SUITE")
