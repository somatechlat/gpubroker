...
from apps.math_core.services import KPICalculator, WorkloadMapper
from apps.math_core.algorithms.topsis import TOPSISEngine
from apps.math_core.benchmarks import BenchmarkRepository
...
class TestKPICalculator:
    ...
    def test_cost_per_token_different_model_sizes(self, kpi_calculator):
        ...
        assert result_70b['cost_per_million_tokens'] > result_7b['cost_per_million_tokens']
...
class TestTOPSISEngine:
    ...
    def test_topsis_basic_calculation(
        self, topsis_engine, sample_decision_matrix, sample_weights, sample_criteria_types
    ):
...
class TestWorkloadMapper:
    ...
    def test_estimate_quality_affects_time(self, workload_mapper):
        ...
        assert result_high['estimated_time_per_unit'] > result_draft['estimated_time_per_unit']
...
class TestMathCoreAPI:
    ...
    async def test_cost_per_token_endpoint(self, api_client):
        ...
        response = await api_client.post('/math/cost-per-token', json={
            'price_per_hour': 1.50,
            'gpu_type': 'RTX 4090',
            'model_size': '7b'
        })
    ...
    async def test_cost_per_gflop_endpoint(self, api_client):
        ...
        response = await api_client.post('/math/cost-per-gflop', json={
            'price_per_hour': 1.50,
            'gpu_type': 'RTX 4090',
            'precision': 'fp32'
        })
    ...
    async def test_topsis_endpoint(self, api_client):
        ...
        response = await api_client.post('/math/topsis', json={
            'decision_matrix': [
                [1.50, 24, 82.6, 0.95],
                [3.50, 80, 19.5, 0.85],
                [0.50, 10, 29.8, 0.99],
    ...
    async def test_estimate_workload_endpoint(self, api_client):
        ...
        response = await api_client.post('/math/estimate-workload', json={
            'workload_type': 'image_generation',
            'quantity': 100,
            'duration_hours': 1.0,
            'quality': 'standard'
    ...
    async def test_ensemble_recommend_endpoint(self, api_client):
        ...
        response = await api_client.post('/math/ensemble-recommend', json={
            'candidate_offers': [
                {
                    'id': '1',
                    'price_per_hour': 1.50,
