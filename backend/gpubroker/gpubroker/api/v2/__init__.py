...
api = NinjaAPI(
    title="GPUBROKER API",
    version="2.0.0",
    description="GPU marketplace platform API - Django Ninja v2",
...
def http_error_handler(request, exc: HttpError):
    ...
    Returns consistent JSON format with appropriate status code.
    """
    trace_id = generate_trace_id()
...
from apps.kpi.api import router as kpi_router
from apps.math_core.api import router as math_router
