...
from .models import Payment, PaymentTransaction, Subscription
...
class SubscriptionService:
        ...
        try:
            ...
            if order_id or transaction_id:
                try:
                    ...
                    logger.warning(
                        f"[Subscription] Unable to link payment transaction: {link_error}"
                    )
    ...
    def activate_subscription(api_key: str) -> dict[str, Any]:
        ...
        if subscription.status not in [
            ...
            return {
                "success": False,
                "error": get_message("subscription.already_active"),
            }
    ...
    def get_dashboard_data() -> dict[str, Any]:
        ...
        total_revenue = subscriptions.aggregate(total=Sum("amount_usd"))[
            "total"
        ] or Decimal("0.00")
        ...
        return {
            ...
            "revenue": {
                ...
                "average": round(
                    float(total_revenue)
                    / max(subscriptions.filter(amount_usd__gt=0).count(), 1),
                    2,
                ),
                "growth": "+23%",
            },
