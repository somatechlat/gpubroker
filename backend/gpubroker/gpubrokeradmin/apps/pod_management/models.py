...
class Pod(models.Model):
    ...
    owner = models.ForeignKey(
        settings.AUTH_USER_MODEL, on_delete=models.PROTECT, related_name="owned_pods"
    )
...
class PodConfiguration(models.Model):
    ...
    pod = models.ForeignKey(
        Pod, on_delete=models.CASCADE, related_name="configurations"
    )
    ...
    mode = models.CharField(
        max_length=20, choices=Pod.Mode.choices, blank=True, null=True
    )
...
class PodDeploymentLog(models.Model):
    ...
    id = models.UUIDField(primary_key=True, default=uuid.uuid4, editable=False)
    pod = models.ForeignKey(
        Pod, on_delete=models.CASCADE, related_name="deployment_logs"
    )
