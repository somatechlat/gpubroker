...
class StripeConfig:
    ...
    def secret_key(self) -> str:
        ...
        if config.mode.is_live:
            return vault.get_secret("gpubroker/stripe", "secret_key_live") or os.getenv(
                "STRIPE_SECRET_KEY_LIVE", ""
            )
        return vault.get_secret("gpubroker/stripe", "secret_key_test") or os.getenv(
    ...
    def webhook_secret(self) -> str:
        """Get webhook secret."""
        return vault.get_secret("gpubroker/stripe", "webhook_secret") or os.getenv(
...
class PayPalConfig:
    ...
    def client_id(self) -> str:
        if config.mode.is_live:
            return vault.get_secret("gpubroker/paypal", "client_id_live") or os.getenv(
                "PAYPAL_CLIENT_ID_LIVE", ""
            )
        return vault.get_secret("gpubroker/paypal", "client_id_sandbox") or os.getenv(
    ...
    def client_secret(self) -> str:
        if config.mode.is_live:
            return vault.get_secret(
                "gpubroker/paypal", "client_secret_live"
            ) or os.getenv("PAYPAL_CLIENT_SECRET_LIVE", "")
        return vault.get_secret(
            "gpubroker/paypal", "client_secret_sandbox"
        ) or os.getenv("PAYPAL_CLIENT_SECRET_SANDBOX", "")
...
class DatabaseConfig:
    ...
    @property
    def url(self) -> str:
        return vault.get_secret("gpubroker/database", "url") or os.getenv(
            "DATABASE_URL", ""
        )
...
class AWSConfig:
    ...
    def access_key_id(self) -> str:
        return vault.get_secret("gpubroker/aws", "access_key_id") or os.getenv(
            "AWS_ACCESS_KEY_ID", ""
        )

    @property
    def secret_access_key(self) -> str:
        return vault.get_secret("gpubroker/aws", "secret_access_key") or os.getenv(
...
class GPUProviderConfig:
    ...
    def runpod_api_key(self) -> str:
        return vault.get_secret("gpubroker/gpu", "runpod_api_key") or os.getenv(
            "RUNPOD_API_KEY", ""
        )

    @property
    def vastai_api_key(self) -> str:
        return vault.get_secret("gpubroker/gpu", "vastai_api_key") or os.getenv(
            "VASTAI_API_KEY", ""
        )

    @property
    def lambda_api_key(self) -> str:
        return vault.get_secret("gpubroker/gpu", "lambda_api_key") or os.getenv(
            "LAMBDA_API_KEY", ""
        )
