...
import requests
from gpubrokeradmin.apps.subscriptions.models import Payment, PaymentTransaction
...
class PayPalService:
    ...
    def create_order(
            ...
            "purchase_units": [
                {
                    ...
                    "description": self.PLAN_DESCRIPTIONS.get(
                        plan, "GPUBROKER POD Subscription"
                    ),
                    "amount": {
                        "currency_code": "USD",
        ...
        try:
            ...
            return {
                "success": False,
                "error": get_message("paypal.api_error", error=str(e)),
            }
        ...
        try:
            ...
            if response.status_code in [200, 201]:
                ...
                if status == "COMPLETED":
                    ...
                    capture_id = ""
                    payments = capture_data.get("purchase_units", [{}])[0].get(
                        "payments", {}
                    )
                    ...
                    logger.info(
                        f"PayPal payment captured: {capture_id} for order {order_id}"
                    )
                    return {
                        "success": True,
                        "status": "COMPLETED",
                ...
                self._persist_transaction(
                    ...
                    error_message=get_message(
                        "paypal.capture_not_completed", status=status
                    ),
                )
                return {
                    "success": False,
            ...
            return {
                ...
                "error": error_data.get(
                    "message", get_message("paypal.capture_failed")
                ),
                "details": error_data,
            }
        except requests.RequestException as e:
    ...
    def get_pending_transaction(self, order_id: str) -> dict | None:
        """
