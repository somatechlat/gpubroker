...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(
            self.style.SUCCESS("🔐 Initializing configuration from Vault...")
        )
        ...
        try:
            ...
            with transaction.atomic():
                ...
                if db_secret:
                    ...
                    self.stdout.write(
                        self.style.SUCCESS("✅ Database configuration stored")
                    )
                ...
                if redis_secret:
                    ...
                    self.stdout.write(
                        self.style.SUCCESS("✅ Cache configuration stored")
                    )
                ...
                if django_secret:
                    ...
                    self.stdout.write(
                        self.style.SUCCESS("✅ Django configuration stored")
                    )
            ...
            self.stdout.write(
                self.style.SUCCESS(
                    "All secrets are now stored in Django database (encrypted)"
                )
            )
            self.stdout.write(
                self.style.WARNING(
            ...
            self.stdout.write(
                self.style.ERROR(f"❌ Failed to initialize configuration: {e}")
            )
            raise
    ...
    def _get_vault_secret(self, vault_addr, headers, path):
        ...
        try:
            response = requests.get(
                f"{vault_addr}/v1/{path}", headers=headers, timeout=10
            )
            response.raise_for_status()
            data = response.json()
