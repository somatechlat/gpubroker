...
class DatabaseConfiguration(models.Model):
    ...
    password = EncryptedCharField(
        max_length=255,
        help_text="Encrypted database password"
    )
    
    # Connection options
    conn_max_age = models.IntegerField(
        default=60,
        help_text="Connection max age in seconds"
    )
...
class CacheConfiguration(models.Model):
    ...
    def get_django_config(self):
        ...
        location = f"redis://:{self.password}@{self.host}:{self.port}/{self.db}" if self.password else f"redis://{self.host}:{self.port}/{self.db}"
