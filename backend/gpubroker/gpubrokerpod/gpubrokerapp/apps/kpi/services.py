...
from gpubrokerpod.gpubrokerapp.apps.providers.models import (
    GPUOffer,
    Provider,
    ProviderHealthCheck,
)
...
class KPIEngine:
    ...
    def _find_gpu_key(self, gpu_type: str, mapping: dict[str, Any]) -> str | None:
...
async def get_kpi_overview() -> dict[str, Any]:
    ...
    avg_price_result = await GPUOffer.objects.aaggregate(
    ...
    try:
        health_agg = await ProviderHealthCheck.objects.aaggregate(
            avg_latency=Avg("latency_ms"), avg_uptime=Avg("uptime_pct")
        )
...
async def get_gpu_metrics(
    gpu_type: str, provider_name: str | None = None
) -> dict[str, Any]:
    """
    ...
    agg = await queryset.aaggregate(avg_price=Avg("price_per_hour"), count=Count("id"))
...
async def get_provider_kpis(provider_name: str) -> dict[str, Any]:
    ...
    reliability_score = (
        float(provider.reliability_score) if provider.reliability_score else 0.5
    )
...
async def get_market_insights() -> dict[str, Any]:
    ...
    cheapest = (
        await GPUOffer.objects.select_related("provider")
    ...
    if most_expensive:
        most_expensive_offer = {
            ...
            "provider": (
                most_expensive.provider.name if most_expensive.provider else "unknown"
            ),
        }
