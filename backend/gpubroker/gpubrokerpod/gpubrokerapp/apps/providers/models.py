...
class GPUOffer(models.Model):
    ...
    provider = models.ForeignKey(
        Provider, on_delete=models.CASCADE, related_name="offers"
    )
    ...
    storage_type = models.CharField(
        max_length=20, choices=STORAGE_TYPE_CHOICES, default="SSD"
    )
...
class PriceHistory(models.Model):
    ...
    offer = models.ForeignKey(
        GPUOffer, on_delete=models.CASCADE, related_name="price_history"
    )
    price_per_hour = models.DecimalField(max_digits=8, decimal_places=4)
...
class KPICalculation(models.Model):
    ...
    id = models.UUIDField(primary_key=True, default=uuid.uuid4, editable=False)
    offer = models.ForeignKey(
        GPUOffer, on_delete=models.CASCADE, related_name="kpi_calculations"
    )
...
class ProviderHealthCheck(models.Model):
    ...
    id = models.UUIDField(primary_key=True, default=uuid.uuid4, editable=False)
    provider = models.ForeignKey(
        Provider, on_delete=models.CASCADE, related_name="health_checks"
    )
