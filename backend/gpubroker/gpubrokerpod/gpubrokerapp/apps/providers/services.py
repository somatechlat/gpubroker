...
async def get_cached_featured_offers(limit: int) -> Optional[Dict[str, Any]]:
    """
...
async def get_featured_offers(limit: int = 6) -> Dict[str, Any]:
    ...
    # Try database first
    try:
        queryset = GPUOffer.objects.select_related('provider').filter(
            availability_status__in=['available', 'limited']
        ...
        for provider_name, items in all_results.items():
            ...
            for item in items:
                if item.get("availability", "").lower() in ["available", "limited"]:
                    ...
                    item["availability_score"] = get_availability_score(item.get("availability", ""))
...
async def browse_gpu_offers(
    ...
    for offer in offers:
        ...
        items.append({
            ...
            "min_rental_hours": offer.min_rental_time / 60 if offer.min_rental_time else 1.0,
            "max_rental_hours": offer.max_rental_time / 60 if offer.max_rental_time else None,
            "network_speed_gbps": float(offer.network_speed_gbps) if offer.network_speed_gbps else None,
    ...
    if has_next and items:
        last_item = items[-1]
        next_cursor = encode_cursor(last_item["id"], last_item.get(sort_field, ""), page + 1)
...
async def _calculate_aggregations(queryset, provider_logos: Dict[str, str]) -> Dict[str, Any]:
    """Calculate aggregations for filter facets."""
    ...
    try:
        ...
        # Availability counts
        avail_counts = {}
        async for item in queryset.values('availability_status').annotate(count=Count('id')):
...
async def calculate_topsis_rankings(items: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    """
    ...
    performance_estimates = {
        "H100": 100, "A100": 80, "L40": 75, "A6000": 70,
        "RTX 4090": 65, "RTX 4080": 55, "A10": 50,
        "RTX 3090": 45, "RTX 3080": 40, "V100": 35,
        "T4": 25, "L4": 30, "RTX 3070": 30,
    }
...
async def get_topsis_ranking(
    ...
    # Fetch offers
    offers = []
    async for offer in GPUOffer.objects.select_related('provider').filter(id__in=offer_ids):
        offers.append({
            "id": str(offer.id),
            "price_per_hour": float(offer.price_per_hour),
            "memory_gb": offer.gpu_memory_gb or 0,
    ...
    if len(offers) < 2:
        return {
            "rankings": [{"offer_id": o["id"], "score": 1.0, "rank": 1} for o in offers],
            "ideal_solution": {},
            "anti_ideal_solution": {},
            "criteria_used": [],
            "weights_used": {},
    ...
    weights = custom_weights or {"price_per_hour": 0.4, "memory_gb": 0.35, "availability_score": 0.25}
    ...
    try:
        ...
        return {
            "rankings": [{"offer_id": o["id"], "score": 0.5, "rank": i + 1} for i, o in enumerate(offers)],
            "ideal_solution": {},
            "anti_ideal_solution": {},
            "criteria_used": criteria,
            "weights_used": weights,
