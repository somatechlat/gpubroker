...
class LambdaLabsAdapter(BaseProviderAdapter):
    ...
    TPS_MAPPING = {
        "h100": 12000, "a100": 8000, "a6000": 2200,
        "4090": 4500, "3090": 3500, "v100": 3000,
    }
    ...
    def _parse_offers(self, data: Dict) -> List[ProviderOffer]:
        ...
        for name, info in instance_types.items():
            ...
            gpu_name = specs.get("gpus", [{}])[0].get("name", name) if specs.get("gpus") else name
            memory_gb = specs.get("gpus", [{}])[0].get("memory_gib", 0) if specs.get("gpus") else 0
            
            availability = "available" if info.get("regions_with_capacity_available") else "unavailable"
