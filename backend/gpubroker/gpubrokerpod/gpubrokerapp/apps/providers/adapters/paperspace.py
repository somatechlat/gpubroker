...
class PaperspaceAdapter(BaseProviderAdapter):
        ...
        try:
            ...
            async with httpx.AsyncClient(timeout=30.0) as client:
                response = await client.get(
                    f"{self.BASE_URL}/machines/getAvailability", headers=headers
                )
    ...
    def _parse_offers(self, data: dict) -> list[ProviderOffer]:
        ...
        for region_data in data if isinstance(data, list) else [data]:
            region = (
                region_data.get("region", "NY1")
                if isinstance(region_data, dict)
                else "NY1"
            )
            available = (
                region_data.get("available", {})
                if isinstance(region_data, dict)
                else {}
            )
            ...
            for machine_type, specs in machine_types.items():
                availability = (
                    "available" if available.get(machine_type, False) else "limited"
                )
    ...
    async def validate_credentials(self, credentials: dict[str, str]) -> bool:
        ...
        try:
            async with httpx.AsyncClient(timeout=10.0) as client:
                response = await client.get(
                    f"{self.BASE_URL}/users/getUser", headers={"X-Api-Key": api_key}
                )
