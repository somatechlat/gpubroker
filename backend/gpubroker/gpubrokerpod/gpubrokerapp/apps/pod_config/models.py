...
class PodConfiguration(models.Model):
    ...
    agent_zero_enabled = models.BooleanField(
        default=False,
        help_text="Enable Agent Zero agentic layer (ADMIN ONLY)"
    )
...
class PodParameter(models.Model):
    ...
    sandbox_value = models.TextField(
        blank=True, 
        null=True,
        help_text="Value used in SANDBOX mode"
    )
    live_value = models.TextField(
        blank=True, 
        null=True,
        help_text="Value used in LIVE mode"
    )
    ...
    is_required = models.BooleanField(
        default=False,
        help_text="If true, parameter must have a value"
    )
    ...
    def get_value(self, mode: str = None) -> Optional[str]:
        """
    ...
    def get_typed_value(self, mode: str = None) -> Any:
        ...
        try:
            if self.parameter_type == self.ParameterType.INTEGER:
                return int(value)
            elif self.parameter_type == self.ParameterType.FLOAT:
                return float(value)
            elif self.parameter_type == self.ParameterType.BOOLEAN:
                return value.lower() in ('true', '1', 'yes', 'on')
            elif self.parameter_type == self.ParameterType.JSON:
    ...
    def set_value(self, value: Any, mode: str, user=None, reason: str = '') -> bool:
        ...
        self.save(update_fields=['sandbox_value' if mode == PodConfiguration.Mode.SANDBOX else 'live_value', 'updated_at'])
...
class ParameterAuditLog(models.Model):
    ...
    mode_affected = models.CharField(
        max_length=10,
        help_text="sandbox, live, or both"
    )
    ...
    change_type = models.CharField(
        max_length=20,
        default='update',
        help_text="create, update, delete, mode_switch"
    )
