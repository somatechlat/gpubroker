...
def list_pods(
    request: HttpRequest,
    mode: Optional[str] = Query(None, description="Filter by mode (sandbox/live)"),
    ...
    return PodConfigListResponseSchema(
        total=total,
        items=[PodConfigResponseSchema.from_orm(pod) for pod in items]
    )
...
def list_parameters(
    request: HttpRequest,
    pod_id: str,
    parameter_type: Optional[str] = Query(None, description="Filter by type"),
...
def update_parameter(request: HttpRequest, pod_id: str, key: str, data: ParameterUpdateSchema):
    """
...
def list_audit_logs(
    ...
    mode_affected: Optional[str] = Query(None, description="Filter by mode affected"),
    page: int = Query(1, ge=1),
    ...
    return AuditLogListResponseSchema(
        total=total,
        items=[AuditLogResponseSchema.from_orm(log) for log in items]
    )
...
def bulk_import_parameters(request: HttpRequest, pod_id: str, data: BulkImportSchema):
    ...
    logger.info(f"Bulk import for {pod_id}: {created} created, {updated} updated, {skipped} skipped")
