...
class GPUBenchmark(models.Model):
    ...
    tflops_int8 = models.FloatField(
        null=True, blank=True, help_text="INT8 performance in TFLOPS"
    )
    ...
    tokens_per_second_13b = models.IntegerField(
        null=True, blank=True, help_text="Tokens/sec for Llama-2 13B"
    )
    tokens_per_second_70b = models.IntegerField(
        null=True, blank=True, help_text="Tokens/sec for Llama-2 70B"
    )

    # Image Generation (Stable Diffusion XL, 1024x1024, 30 steps)
    image_gen_per_minute = models.IntegerField(
        null=True, blank=True, help_text="Images per minute for SDXL"
    )

    # Metadata
    architecture = models.CharField(
        max_length=50, choices=ARCHITECTURE_CHOICES, blank=True, null=True
    )
    tdp_watts = models.IntegerField(
        null=True, blank=True, help_text="Thermal Design Power in Watts"
    )
    source = models.CharField(max_length=255, help_text="Data source citation")
    verified_at = models.DateTimeField(
        null=True, blank=True, help_text="When benchmark was last verified"
    )
