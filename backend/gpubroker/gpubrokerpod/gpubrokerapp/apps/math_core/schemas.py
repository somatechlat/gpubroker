...
class CostPerTokenRequest(Schema):
    ...
    model_size: str = Field(
        "7b", description="LLM model size (e.g., '7b', '13b', '70b')"
    )
...
class GPUBenchmarkSchema(Schema):
    ...
    tflops_fp32: float
    tflops_fp16: float
    tflops_int8: float | None = None
    memory_bandwidth_gbps: float
    vram_gb: int
    tokens_per_second_7b: int
    tokens_per_second_13b: int | None = None
    tokens_per_second_70b: int | None = None
    image_gen_per_minute: int | None = None
    source: str
