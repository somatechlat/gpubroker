...
class KPICalculator:
    ...
    def __init__(self, benchmark_repository: Optional[BenchmarkRepository] = None):
    ...
    def _estimate_tflops(self, gpu_type: str, precision: str) -> float:
        ...
        base_estimates = {
            "H100": 60.0, "A100": 19.0, "A10": 30.0, "A6000": 38.0,
            "V100": 15.0, "4090": 80.0, "4080": 48.0, "3090": 35.0,
            "3080": 29.0, "T4": 8.0, "L40": 90.0, "L4": 30.0,
        }
...
class WorkloadMapper:
    ...
    def estimate(
        ...
        if workload_type == "image_generation":
            return self._estimate_image_generation(profile, quantity, duration_hours, quality)
        elif workload_type == "llm_inference":
            return self._estimate_llm_inference(profile, quantity, duration_hours, model_name)
        elif workload_type == "training":
            return self._estimate_training(profile, quantity, duration_hours, model_name)
        else:
    ...
    def _estimate_training(
        ...
        estimated_epochs = max(1, int(duration_hours / (hours_per_epoch * max(dataset_size_k, 0.1))))
