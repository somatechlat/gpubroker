...
class AIAssistantService:
    ...
    async def chat(
        ...
        try:
            ...
            result = await client.invoke(
                messages=messages, session_id=user_id, tenant=None
            )
            ...
            if user_id:
                try:
                    ...
                    history_payload = (
                        hist.get("items") if isinstance(hist, dict) else hist
                    )
        ...
        try:
            async with httpx.AsyncClient(timeout=5.0) as http:
                ...
                prov_resp = await http.get(
                    f"{PROVIDER_API_URL}/providers", params=params
                )
                ...
                rec_resp = await http.post(
                    f"{MATH_CORE_URL}/math/ensemble-recommend", json=payload
                )
    ...
    def parse_workload(self, text: str) -> dict[str, Any]:
        ...
        if any(
            kw in text_lower
            for kw in ["token", "chat", "llm", "inference", "gpt", "llama"]
        ):
            workload_type = "llm_inference"
        elif any(
            kw in text_lower
            for kw in ["image", "picture", "stable diffusion", "sdxl", "dall"]
        ):
            workload_type = "image_generation"
        elif any(
            kw in text_lower for kw in ["train", "fine-tune", "finetune", "training"]
        ):
...
WORKLOAD_TEMPLATES = [
    {
        ...
        "default_values": {"workload_type": "image_generation", "batch_size": 1},
    ...
    },
    {
        ...
        "default_values": {"workload_type": "llm_inference", "batch_size": 1},
    ...
    },
    {
        ...
        "default_values": {"workload_type": "training", "batch_size": 8},
    ...
    },
    {
        ...
        "default_values": {"workload_type": "video_processing", "batch_size": 1},
    ...
    },
    {
        ...
        "default_values": {"workload_type": "data_processing", "batch_size": 1},
...
class WorkloadTemplateService:
    ...
    def apply_template(
        ...
        estimated_requirements = self._estimate_requirements(
            template_id, workload_profile
        )
    ...
    def _estimate_requirements(
        self, template_id: str, workload_profile: dict[str, Any]
    ) -> dict[str, Any]:
        """
    ...
    def _estimate_training(self, profile: dict[str, Any]) -> dict[str, Any]:
        ...
        return {
            ...
            "recommended_vram_gb": rec_vram,
            "recommended_gpu_tiers": (
                ["H100", "A100", "A6000"] if rec_vram <= 80 else ["8xH100", "8xA100"]
            ),
            "multi_gpu_required": rec_vram > 80,
            "estimated_cost_range": {
...
class AIContextAwarenessService:
    ...
    def analyze_search(
        ...
        if visible_offers:
            ...
            prices = [
                o.get("price_per_hour", 0)
                for o in visible_offers
                if o.get("price_per_hour")
            ]
            vrams = [
                o.get("gpu_memory_gb", 0)
                for o in visible_offers
                if o.get("gpu_memory_gb")
            ]
            ...
            if prices:
                ...
                budget_offers = [
                    o
                    for o in visible_offers
                    if o.get("price_per_hour", 999) < avg_price * 0.7
                ]
                if budget_offers:
                    insights.append(
                        {
                            ...
                            "confidence": 0.85,
                            "related_offers": [
                                o.get("id") for o in budget_offers[:3] if o.get("id")
                            ],
                        }
                    )
            ...
            if vrams:
                ...
                high_vram = [
                    o for o in visible_offers if o.get("gpu_memory_gb", 0) >= 48
                ]
                    ...
                    insights.append(
                        {
                            ...
                            "confidence": 0.8,
                            "related_offers": [
                                o.get("id") for o in high_vram if o.get("id")
                            ],
                        }
                    )
            ...
            available = [
                o for o in visible_offers if o.get("availability_status") == "available"
            ]
            ...
            if visible_offers:
                if sort_by == "price_per_hour" and sort_order == "asc":
                    best_match = min(
                        visible_offers, key=lambda x: x.get("price_per_hour", 999)
                    )
                elif sort_by == "gpu_memory_gb" and sort_order == "desc":
                    best_match = max(
                        visible_offers, key=lambda x: x.get("gpu_memory_gb", 0)
                    )
                else:
                    ...
                    def score(o):
                        ...
                        avail = (
                            1 if o.get("availability_status") == "available" else 0.5
                        )
                        # Lower price better, higher VRAM better, available better
                        return (
                            (vram / max(vrams) if vrams and max(vrams) > 0 else 0) * 0.4
                            + (
                                1 - price / max(prices)
        ...
        if not filters:
            suggestions.append(
                "Add filters to narrow down results (e.g., GPU type, region, price range)"
            )
        else:
            ...
            if not filters.get("region"):
                suggestions.append(
                    "Add a region filter to find nearby providers with lower latency"
                )
            if (
                filters.get("price_max")
                and prices
                and min(prices) > filters.get("price_max", 0) * 0.8
            ):
                suggestions.append(
                    "Consider increasing your price limit for more options"
                )
        ...
        if visible_offers:
            ...
            if prices:
                summary += (
                    f"Prices range from ${min(prices):.2f} to ${max(prices):.2f}/hr."
                )
        else:
    ...
    async def context_aware_chat(
        ...
        if screen_context:
            ...
            if visible_offers:
                ...
                if any(
                    ...
                    enriched_context["visible_offers"] = visible_offers[
                        :10
            ...
            if selected_offer_id:
                selected = next(
                    (o for o in visible_offers if o.get("id") == selected_offer_id),
                    None,
                )
                if selected:
            ...
            suggested_filters = self._detect_filter_suggestions(
                message, filters, visible_offers
            )
        ...
        if screen_context and screen_context.get("visible_offers"):
            for offer in screen_context["visible_offers"]:
                ...
                if offer_id and (
                    offer_id in reply or gpu_type.lower() in reply.lower()
                ):
        ...
        return {
            ...
            "referenced_offers": referenced_offers or None,
    ...
    def _summarize_offers(self, offers: list[dict[str, Any]]) -> dict[str, Any]:
        ...
        return {
            ...
            "available_count": len(
                [o for o in offers if o.get("availability_status") == "available"]
            ),
        }
    ...
    def _detect_filter_suggestions(
        ...
        # Price mentions
        if (
            "cheap" in message_lower
            or "budget" in message_lower
            or "affordable" in message_lower
        ):
            if visible_offers:
                prices = [
                    o.get("price_per_hour", 0)
        ...
        if (
            "expensive" in message_lower
            or "premium" in message_lower
            or "high-end" in message_lower
        ):
            if visible_offers:
                prices = [
                    o.get("price_per_hour", 0)
    ...
    def _generate_rule_based_response(
        ...
        if any(kw in message_lower for kw in ["recommend", "suggest", "best", "which"]):
            ...
            if summary:
                ...
                if price_range:
                    return (
                        f"Based on your current search showing {count} offers, "
        ...
        if any(kw in message_lower for kw in ["price", "cost", "expensive", "cheap"]):
            ...
            if price_range:
                return (
                    f"Current prices range from ${price_range.get('min', 0):.2f} to ${price_range.get('max', 0):.2f}/hr. "
        ...
        if any(
            kw in message_lower
            for kw in ["available", "availability", "now", "immediately"]
        ):
            summary = context.get("visible_offers_summary", {})
