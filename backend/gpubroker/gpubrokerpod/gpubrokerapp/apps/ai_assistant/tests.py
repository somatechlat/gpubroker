...
import pytest
from gpubrokerpod.gpubrokerapp.apps.ai_assistant.services import (
    WORKLOAD_TEMPLATES,
...
class TestWorkloadParsing:
    ...
    def test_parse_duration(self):
        ...
        result = ai_assistant_service.parse_workload("Generate images for 4 hours")

        assert result["duration"] == "4 hour"

    def test_parse_region(self):
        """Test parsing region from text."""
        result = ai_assistant_service.parse_workload("I need a GPU in us-east region")

        assert result["region"] == "us-east"

    def test_parse_quality_high(self):
        """Test parsing high quality setting."""
        result = ai_assistant_service.parse_workload("Generate high quality images")

        assert result["quality"] == "high"

    def test_parse_quality_draft(self):
        """Test parsing draft quality setting."""
        result = ai_assistant_service.parse_workload("Generate draft images fast")

        assert result["quality"] == "draft"

    def test_parse_default_workload_type(self):
        """Test default workload type for ambiguous input."""
        result = ai_assistant_service.parse_workload("I need a GPU for my project")
...
class TestTemplateApplication:
    ...
    def test_apply_template_with_defaults(self):
        ...
        result = workload_template_service.apply_template(
            template_id="image_generation", answers={}  # No answers, use all defaults
        )
    ...
    def test_apply_template_unknown_id(self):
        ...
        with pytest.raises(ValueError, match="Unknown template"):
            workload_template_service.apply_template(
                template_id="nonexistent", answers={}
            )
    ...
    def test_apply_template_validation_min(self):
        ...
        with pytest.raises(ValueError, match="below minimum"):
            workload_template_service.apply_template(
                template_id="image_generation", answers={"quantity": 0}  # Min is 1
            )
...
class TestRequirementEstimation:
    ...
    def test_image_gen_vram_by_resolution(self):
        ...
        result_draft = workload_template_service.apply_template(
            "image_generation", {"quality": "512x512 (draft)"}
        )
        result_ultra = workload_template_service.apply_template(
            "image_generation", {"quality": "2048x2048 (ultra)"}
        )

        # Ultra should require more VRAM
        assert (
            result_ultra["estimated_requirements"]["min_vram_gb"]
    ...
    def test_llm_vram_by_model_size(self):
        ...
        result_7b = workload_template_service.apply_template(
            "llm_inference", {"model_name": "7B parameters"}
        )
        result_70b = workload_template_service.apply_template(
            "llm_inference", {"model_name": "70B parameters"}
        )

        # 70B should require more VRAM
        assert (
            result_70b["estimated_requirements"]["min_vram_gb"]
    ...
    def test_llm_vram_with_quantization(self):
        ...
        assert (
            result_int4["estimated_requirements"]["min_vram_gb"]
    ...
    def test_training_vram_by_type(self):
        ...
        assert (
            result_full["estimated_requirements"]["min_vram_gb"]
    ...
    def test_cost_range_estimation(self):
        ...
        result = workload_template_service.apply_template(
            "image_generation", {"quantity": 100, "duration_hours": 2}
        )
...
class TestAIAssistantAPI:
    ...
    async def test_apply_template_invalid_template(self, api_client):
        ...
        response = await api_client.post(
            "/ai/templates/apply", json={"template_id": "nonexistent", "answers": {}}
        )
    ...
    async def test_parse_workload_endpoint(self, api_client):
        ...
        response = await api_client.post(
            "/ai/parse-workload",
            json={"text": "I need to generate 500 images using Stable Diffusion"},
        )
...
from gpubrokerpod.gpubrokerapp.apps.ai_assistant.services import ai_context_service
...
class TestAnalyzeSearch:
    ...
    def test_analyze_empty_search(self):
        ...
        result = ai_context_service.analyze_search(
            screen_context={"current_filters": {}, "visible_offers": []}
        )
    ...
    def test_analyze_search_price_range_insight(self):
        ...
        result = ai_context_service.analyze_search(
            screen_context={"current_filters": {}, "visible_offers": offers}
        )

        # Should have price range insight since 4.50 > 0.30 * 3
        price_insights = [
            i for i in result["insights"] if "price" in i["title"].lower()
        ]
    ...
    def test_analyze_search_budget_friendly_insight(self):
        ...
        budget_insights = [
            i for i in result["insights"] if "budget" in i["title"].lower()
        ]
    ...
    def test_analyze_search_availability_warning(self):
        ...
        avail_insights = [
            i for i in result["insights"] if "availability" in i["title"].lower()
        ]
...
class TestContextAwarenessAPI:
    ...
    async def test_context_chat_without_context(self, api_client):
        ...
        response = await api_client.post(
            "/ai/context-chat", json={"message": "Hello, what can you help me with?"}
        )
