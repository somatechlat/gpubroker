    ...
    def estimate_cost(
        ...
        if provider:
            limits = ProviderLimits.objects.filter(
                provider=provider, gpu_type=gpu_type
            ).first()
    ...
    def validate_config(
        ...
        if not limits:
            ...
            if not any_limits:
                ...
                errors.append(
                    f"Provider '{provider}' does not support GPU type '{gpu_type}'"
                )
...
class PodConfigurationService:
    ...
    def create_config(
        ...
        with transaction.atomic():
            ...
            config = PodDeploymentConfig.objects.create(
                ...
                estimated_price_per_month=Decimal(
                    str(cost_estimate["price_per_month"])
                ),
    ...
    def update_config(
        self, config_id: UUID, user_id: UUID, **updates
    ) -> PodDeploymentConfig:
        """
        ...
        with transaction.atomic():
            ...
            config.estimated_price_per_hour = Decimal(
                str(cost_estimate["price_per_hour"])
            )
            config.estimated_price_per_day = Decimal(
                str(cost_estimate["price_per_day"])
            )
            config.estimated_price_per_month = Decimal(
                str(cost_estimate["price_per_month"])
            )
            ...
            PodDeploymentLog.objects.create(
                ...
                message="Pod configuration updated",
...
class DeploymentService:
    ...
    def deploy(
        ...
        if not config.can_deploy():
            raise ValueError(
                f"Cannot deploy: status={config.status}, is_valid={config.is_valid}"
            )
    ...
    def get_status(self, config_id: UUID, user_id: UUID) -> dict[str, Any]:
        ...
        return {
            ...
            "error_message": (
                config.validation_errors[0] if config.validation_errors else None
            ),
        }
