...
class StripeService:
    ...
    async def confirm_setup_intent(self, setup_intent_id: str) -> dict | None:
        """Get SetupIntent status after confirmation."""
        if not self.is_configured:
            return None

        try:
            setup_intent = await sync_to_async(stripe.SetupIntent.retrieve)(
                setup_intent_id
            )
            return {
                "id": setup_intent.id,
    ...
    async def get_payment_method(self, payment_method_id: str) -> dict | None:
    ...
    async def get_upcoming_invoice(self, customer_id: str) -> dict | None:
