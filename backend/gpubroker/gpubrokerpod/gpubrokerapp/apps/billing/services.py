...
async def get_plan_by_tier(tier: str) -> dict[str, Any] | None:
    """
    ...
    try:
        plan = await sync_to_async(
            Plan.objects.filter(tier=tier, is_active=True).first
        )()
        if not plan:
            return None
        return {
    ...
    try:
        ...
        if subscription.stripe_subscription_id:
            try:
                ...
                logger.error(
                    f"Failed to cancel Stripe subscription {subscription.stripe_subscription_id}: {stripe_error}"
                )
...
async def record_usage(
    ...
    try:
        ...
        if usage_type == UsageRecord.UsageType.TOKEN:
            ...
            await sync_to_async(subscription.save)(
                update_fields=["tokens_used", "updated_at"]
            )
...
async def get_usage_summary(user_id: str) -> dict[str, Any] | None:
    """
