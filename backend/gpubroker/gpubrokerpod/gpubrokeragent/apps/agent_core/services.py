...
from .models import AgentSession, AgentMessage, AgentToolExecution
from ..decisions.models import AgentDecision
from ..budgets.models import AgentBudget, AgentUsageLog
...
class AgentZeroService:
    ...
    async def get_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "budget_percentage": float(budget_used / self.budget_limit * 100) if self.budget_limit > 0 else 0,
    ...
    async def override_decision(
        ...
        if action == 'approve':
            ...
            if result.get('success'):
                ...
                decision.actual_cost = result.get('actual_cost', decision.estimated_cost)
        elif action == 'reject':
    ...
    async def _calculate_budget_used(self) -> Decimal:
        ...
        for d in self._decisions:
            if d.status == 'executed':
                cost = d.actual_cost if d.actual_cost else d.estimated_cost
        ...
        def save():
            AgentDecision.objects.update_or_create(
                ...
                defaults={
                    ...
                    'approved_at': django_timezone.now() if decision.status == 'approved' else None,
                    'rejection_reason': decision.admin_notes if decision.status == 'rejected' else '',
                    'executed_at': django_timezone.now() if decision.status == 'executed' else None,
                    'execution_result': {
                        'actual_cost': decision.actual_cost,
                    } if decision.actual_cost else {},
                }
            )
